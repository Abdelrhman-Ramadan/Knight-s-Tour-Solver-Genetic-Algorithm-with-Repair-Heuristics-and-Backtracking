...
def main():
    ...
    for n in range(4, 9):
        ...
        n_values.append(n)
        solution_counts.append(number_of_solutions)
        number_of_solutions=0

    # Plot the data
    plt.plot(n_values, solution_counts, marker='o')
    plt.xlabel('Chessboard Size (n)')
    plt.ylabel('Number of Solutions')
    plt.xticks(np.arange(4, 9, step=1)) 
    plt.yticks(solution_counts)
    plt.title('KnightsTour Solutions found in 15 seconds , Starting state: ' +str(row)+' , '+ str(col))
    plt.show()
